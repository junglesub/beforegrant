...
@Tag(name = "0-2. 권한사용자 API 안내",
        description = "권한사용자 관련 기능 정의한 RestController.")
@RequestMapping("/api/tbgrantuser")
@RestController
public class TbgrantuserRestController {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    private final String tbgrantCate = "tbgrantuser";
    ...
    @Operation(summary = "권한사용자 생성",
            description = "권한사용자 생성 컨트롤러 <br />"
    ...
    @Operation(summary = "권한사용자 수정",
            description = "권한사용자 수정 컨트롤러 <br />"
    ...
    @Operation(summary = "권한사용자 삭제",
            description = "권한사용자 삭제 컨트롤러 <br />"
    ...
    @Operation(summary = "권한사용자 여러개 삭제",
            description = "권한사용자 여러개 삭제 컨트롤러 <br />"
                    + "@param TbgrantuserDto.DeleteReqDto <br />"
                    + "@return HttpStatus.OK(200) ResponseEntity\\<TbgrantuserDto.CreateResDto\\> <br />"
                    + "@exception 필수 파라미터 누락하였을 때 등 <br />"
    )
    ...
    @Operation(summary = "권한사용자 상세 조회",
            description = "권한사용자 상세 조회 컨트롤러 <br />"
    ...
    @Operation(summary = "권한사용자 목록 전체 조회",
            description = "권한사용자 목록 전체 조회 컨트롤러 <br />"
    ...
    public ResponseEntity<List<TbgrantuserDto.DetailResDto>> list(@Valid TbgrantuserDto.ListReqDto param, @AuthenticationPrincipal PrincipalDetails principalDetails){
        boolean isAdmin = true;
        String reqTbuserId = null; if(principalDetails != null && principalDetails.getTbuser() != null){ reqTbuserId = principalDetails.getTbuser().getId(); }
        TbgrantuserDto.ListServDto newParam = (TbgrantuserDto.ListServDto) TbgrantuserDto.ListServDto.builder().reqTbuserId(reqTbuserId).isAdmin(isAdmin).build().afterBuild(param);

        return ResponseEntity.status(HttpStatus.OK).body(tbgrantuserService.list(newParam));
    }

    @Operation(summary = "권한사용자 목록 페이지 조회",
            description = "권한사용자 목록 페이지 조회 컨트롤러 <br />"
    ...
    public ResponseEntity<DefaultDto.PagedListResDto> plist(@Valid TbgrantuserDto.PagedListReqDto param, @AuthenticationPrincipal PrincipalDetails principalDetails){
        boolean isAdmin = true;
        String reqTbuserId = null; if(principalDetails != null && principalDetails.getTbuser() != null){ reqTbuserId = principalDetails.getTbuser().getId(); }
        TbgrantuserDto.PagedListServDto newParam = (TbgrantuserDto.PagedListServDto) TbgrantuserDto.PagedListServDto.builder().reqTbuserId(reqTbuserId).isAdmin(isAdmin).build().afterBuild(param);

        return ResponseEntity.status(HttpStatus.OK).body(tbgrantuserService.pagedList(newParam));
    }
    @Operation(summary = "권한사용자 목록 스크롤 조회",
            description = "권한사용자 목록 스크롤 조회 컨트롤러 <br />"
    ...
    public ResponseEntity<List<TbgrantuserDto.DetailResDto>> mlist(@Valid TbgrantuserDto.ScrollListReqDto param, @AuthenticationPrincipal PrincipalDetails principalDetails){
        boolean isAdmin = true;
        String reqTbuserId = null; if(principalDetails != null && principalDetails.getTbuser() != null){ reqTbuserId = principalDetails.getTbuser().getId(); }
        TbgrantuserDto.ScrollListServDto newParam = (TbgrantuserDto.ScrollListServDto) TbgrantuserDto.ScrollListServDto.builder().reqTbuserId(reqTbuserId).isAdmin(isAdmin).build().afterBuild(param);
